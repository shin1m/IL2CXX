...
export class WasmRootBuffer {
    ...
    get(index: number): ManagedPointer {
        this._check_in_range(index);
        return <any>this._unsafe_get(index);
    }
	...
	cwraps.il2cxx_wasm_slot_set(<any>this.__offset + (index * 4), <any>value);
    }

    clear(): void {
        if (!this.__offset) return;
        const q = <any>this.__offset + this.__count * 4;
        for (let p = <any>this.__offset; p < q; p += 4) cwraps.il2cxx_wasm_slot_set(p, <any>0);
    }

    release(): void {
        this.clear();
        if (this.__offset && this.__ownsAllocation) Module._free(this.__offset);
    }
