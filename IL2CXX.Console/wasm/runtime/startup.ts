...
export function configureEmscriptenStartup (module: DotnetModuleInternal): void {
    ...
    // execution order == [2] ==
    module.preRun = [() => preInit(userPreInit), () => preRunAsync(userPreRun)];
    // execution order == [4] ==
    ...
    try {
        ...
        if (runtimeHelpers.config.virtualWorkingDirectory) {
            ...
            try {
                ...
                if (!wds) {
                    Module.FS.createPath("/", cwd, true, true);
                } else {
...
async function postRunAsync (userpostRun: ((module:EmscriptenModule) => void)[]) {
    ...
    try {
        ...
        Module["FS"]["createPath"]("/", "usr", true, true);
        Module["FS"]["createPath"]("/", "usr/share", true, true);
