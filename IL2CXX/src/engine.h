...
struct t_engine : recyclone::t_engine
{
	...
	{
		return static_cast<t__object*>(recyclone::t_engine::f_object__allocate(a_size));
	}
	template<typename T>
...
{
	{
		...
		a_thread->v__internal = new t_thread();
	}
	t_slot::t_increments::f_push(a_thread);
	try {
		std::thread([this, a_thread, main = std::move(a_main)]
		{
			...
			t_slot::t_decrements::f_push(a_thread);
...
{
	return static_cast<t_engine*>(recyclone::f_engine());
}
