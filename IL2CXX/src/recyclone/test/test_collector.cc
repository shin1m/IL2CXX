...
t_pair* f_hanoi(t_pair* a_tower, T_move a_move)
{
	...
	})(static_cast<t_pair*>(a_tower->v_tail), f_cons(a_tower, f_cons(nullptr, f_cons(nullptr))), 0, 1, 2);
}

t_object* f_get(t_pair* a_xs, size_t a_i)
{
	return a_i > 0 ? f_get(static_cast<t_pair*>(a_xs->v_tail), a_i - 1) : static_cast<t_object*>(a_xs->v_head);
}

t_pair* f_put(t_pair* a_xs, size_t a_i, t_object* a_x)
{
	return a_i > 0
		? f_cons(a_xs->v_head, f_put(static_cast<t_pair*>(a_xs->v_tail), a_i - 1, a_x))
		: f_cons(a_x, a_xs->v_tail);
}
...
int main(int argc, char* argv[])
{
	...
	auto towers = f_hanoi(
		f_cons(f_symbol("a"sv),
		f_cons(f_symbol("b"sv),
		f_cons(f_symbol("c"sv),
		f_cons(f_symbol("d"sv),
		f_cons(f_symbol("e"sv)
	))))), [](auto a_towers, auto a_from, auto a_to)
	{
		...
		return f_put(f_put(a_towers, a_from, tower->v_tail), a_to,
			f_cons(tower->v_head, f_get(a_towers, a_to))
		);
	});
