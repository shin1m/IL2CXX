...
namespace DemoRenderer.ShapeDrawing
{
    ...
    public class MeshCache : Disposable
    {
        ...
        public readonly ArrayBuffer TriangleBuffer;
        ...
        public MeshCache(WebGL2Context context, BufferPool pool)
        {
            Pool = pool;
        ...
        public async Task InitializeAsync(int initialSizeInVertices = 1 << 22)
        {
            Pool.TakeAtLeast(initialSizeInVertices, out vertices);
            await TriangleBuffer.InitializeAsync<Vector4>(initialSizeInVertices);
        ...
        public async Task FlushPendingUploadsAsync()
        {
            ...
            {
                await TriangleBuffer.AllocateAsync((int)allocator.Capacity);
            }
            for (int i = 0; i < pendingUploads.Count; ++i)
            {
                var upload = pendingUploads[i];
                await TriangleBuffer.UpdateAsync(vertices.Slice(upload.Start, upload.Count).ToArray(), upload.Start);
            }
            pendingUploads.Count = 0;
        ...
        {
            await TriangleBuffer.DisposeAsync();
