...
namespace DemoRenderer.ShapeDrawing
{
    ...
    public class ShapesExtractor : IAsyncDisposable
    {
        public ShapeCache ShapeCache;
        private readonly ParallelLooper looper;
        private readonly BufferPool pool;
        public readonly MeshCache MeshCache;

        public ShapesExtractor(WebGL2Context context, ParallelLooper looper, BufferPool pool, int initialCapacityPerShapeType = 1024)
        {
            ShapeCache = new ShapeCache(initialCapacityPerShapeType, pool);
            this.looper = looper;
            this.pool = pool;
            MeshCache = new MeshCache(context, pool);
        }
        public Task InitializeAsync() => MeshCache.InitializeAsync();
