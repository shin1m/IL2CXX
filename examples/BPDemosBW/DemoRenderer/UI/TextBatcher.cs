...
namespace DemoRenderer.UI
{
    ...
    public class TextBatcher
    {
        ...
        public async Task FlushAsync(Int2 screenResolution, GlyphRenderer renderer)
        {
            foreach (var batch in batches)
            {
                await renderer.RenderAsync(batch.Key, screenResolution, new ArraySegment<GlyphInstance>(batch.Value.Glyphs, 0, batch.Value.GlyphCount));
