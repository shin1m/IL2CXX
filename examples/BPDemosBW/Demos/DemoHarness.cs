...
namespace Demos
{
    public class DemoHarness : IAsyncDisposable
    {
        internal readonly GameLoop loop;
        ...
        private readonly DemoSwapper swapper = new();
        internal readonly DemoSet demoSet = new();
        Demo? demo;
        internal async Task TryChangeToDemoAsync(int demoIndex)
        {
            if (demoIndex >= 0 && demoIndex < demoSet.Count)
            {
                await demo!.DisposeAsync();
                demo = await demoSet.BuildAsync(loop.Context, demoIndex, content, loop.Camera);
        ...
        private readonly SimulationTimeSamples timeSamples;
            ...
            Controls? controls = null)
        {
            ...
            timeSamples = new(512, loop.Pool);
            this.controls = controls ?? Controls.Default;

            font = new(loop.Renderer.Context, content.Load<FontContent>(@"Content\Carlito-Regular.ttf"));
        ...
        {
            demo = await demoSet.BuildAsync(loop.Context, 0, content, loop.Camera);
                ...
                case TimingDisplayMode.Big:
                    {
                        ...
                        description.BodySpan = new Vector2(loop.Renderer.Width, loop.Renderer.Height) - description.BodyMinimum - new Vector2(inset);
                ...
                case TimingDisplayMode.Regular:
                    {
                        ...
                        description.BodyMinimum = new Vector2(loop.Renderer.Width - targetSpan.X - inset, inset);
        ...
        public async Task UpdateAsync(float dt)
        {
            ...
            grabber.Update(demo!.Simulation!, camera, input.MouseLocked, controls.Grab.IsDown(input), incrementalGrabRotation, loop.Renderer.GetNormalizedMousePosition(input.MousePosition));
            ...
            await swapper.CheckForDemoSwapAsync(this);
            ++frameCount;
            if (!controls.SlowTimesteps.IsDown(input) || frameCount % 60 == 0)
            {
                demo.Update(loop.Renderer, camera, input, dt);
            }
        ...
        public void Render(Renderer renderer)
        {
            ...
            renderer.TextBatcher.Write(uiText.Clear().Append("Running in Debug configuration. Compile in Release or, better yet, ReleaseStrip configuration for performance testing."),
                new Vector2((renderer.Width - GlyphBatch.MeasureLength(uiText, font, warningHeight)) * 0.5f, warningHeight), warningHeight, new Vector3(1, 0, 0), font);
#endif            
            float textHeight = 16;
            ...
            if (showControls)
            {
                var penPosition = new Vector2(renderer.Width - textHeight * 6 - 25, renderer.Height - 25);
                ...
                void WriteInstantName(string controlName, IInstantBind control)
                {
                ...
                void WriteHoldableName(string controlName, IHoldableBind control)
                {
                    uiText.Clear().Append(controlName).Append(":");
                    renderer.TextBatcher.Write(uiText, penPosition - new Vector2(GlyphBatch.MeasureLength(uiText, font, textHeight), 0), textHeight, textColor, font);
                    penPosition.Y += lineSpacing;
            ...
            else
            {
                ...
                renderer.TextBatcher.Write(uiText,
                    new Vector2(renderer.Width - inset - GlyphBatch.MeasureLength(uiText, font, textHeight), renderer.Height - inset),
            ...
            else
            {
                ...
                renderer.TextBatcher.Write(
                    ...
                    new Vector2(renderer.Width - inset - GlyphBatch.MeasureLength(uiText, font, timingTextSize), inset), timingTextSize, timingGraph.Description.TextColor, font);
            }
            grabber.Draw(renderer.Lines, loop.Camera, loop.Input.MouseLocked, controls.Grab.IsDown(loop.Input), renderer.GetNormalizedMousePosition(loop.Input.MousePosition));
        ...
        {
            if (demo != null) await demo.DisposeAsync();
            timeSamples.Dispose();
