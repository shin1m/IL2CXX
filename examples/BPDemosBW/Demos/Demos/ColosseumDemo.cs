    ...
    public class ColosseumDemo : Demo
    {
        ...
        public override void Update(Renderer renderer, Camera camera, Input input, float dt)
        {
            if (input.WasPushed("KeyZ"))
            {
                ...
                bulletDescription.Velocity.Linear = camera.GetRayDirection(input.MouseLocked, renderer.GetNormalizedMousePosition(input.MousePosition)) * 400;
                Simulation!.Bodies.Add(bulletDescription);
            }
            else if (input.WasPushed("KeyX"))
            {
                shootiePatootieDescription.Pose.Position = camera.Position;
                shootiePatootieDescription.Velocity.Linear = camera.GetRayDirection(input.MouseLocked, renderer.GetNormalizedMousePosition(input.MousePosition)) * 100;
        ...
        public override void Render(Renderer renderer, Camera camera, Input input, TextBuilder text, Font font)
        {
            ...
            renderer.TextBatcher.Write(text, new Vector2(20, renderer.Height - 20), 16, new Vector3(1, 1, 1), font);
