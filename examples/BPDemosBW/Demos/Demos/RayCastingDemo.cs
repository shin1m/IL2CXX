...
namespace Demos
{
    public class RayCastingDemo : Demo
    {
        ...
        private readonly IntersectionAlgorithm[] algorithms = new IntersectionAlgorithm[2];
        ...
        public override void Update(Renderer renderer, Camera camera, Input input, float dt)
        {
            ...
            if (input.WasPushed("KeyR"))
            {
                shouldRotate = !shouldRotate;
            }
            if (input.WasPushed("KeyF"))
            {
                rotation = 0;
            }
            if (input.WasPushed("KeyT"))
            {
        ...
        public override void Render(Renderer renderer, Camera camera, Input input, TextBuilder text, Font font)
        {
            ...
            renderer.TextBatcher.Write(text, new Vector2(32, renderer.Height - 192), 16, new Vector3(1), font);
            renderer.TextBatcher.Write(text.Clear().Append("Demo specific controls:"), new Vector2(32, renderer.Height - 176), 16, new Vector3(1), font);
            WriteControl("Change ray source", text.Clear().Append("1 through ").Append(raySources.Count), renderer.Height - 160, renderer.TextBatcher, font);
            WriteControl("Toggle cycling", text.Clear().Append("C"), renderer.Height - 144, renderer.TextBatcher, font);
            WriteControl("Toggle rotation", text.Clear().Append("R"), renderer.Height - 128, renderer.TextBatcher, font);
            WriteControl("Reset rotation", text.Clear().Append("F"), renderer.Height - 112, renderer.TextBatcher, font);
            WriteControl("Toggle threading", text.Clear().Append("T"), renderer.Height - 96, renderer.TextBatcher, font);

            renderer.TextBatcher.Write(text.Clear().Append("Ray count: ").Append(testRays.Count), new Vector2(32, renderer.Height - 80), 16, new Vector3(1), font);
            renderer.TextBatcher.Write(text.Clear().Append("Time (us):"), new Vector2(128, renderer.Height - 64), 16, new Vector3(1), font);
            renderer.TextBatcher.Write(text.Clear().Append("Rays per second:"), new Vector2(224, renderer.Height - 64), 16, new Vector3(1), font);
            renderer.TextBatcher.Write(text.Clear().Append("Relative speed:"), new Vector2(350, renderer.Height - 64), 16, new Vector3(1), font);
            ...
            for (int i = 0; i < algorithms.Length; ++i)
            {
                ...
                WriteResults(algorithms[i].Name, stats.Average, baseStats.Average, renderer.Height - (baseHeight - 16 * i), renderer.TextBatcher, text, font);
            }
