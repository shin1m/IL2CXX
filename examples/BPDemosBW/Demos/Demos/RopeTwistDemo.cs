...
namespace Demos.Demos
{
    ...
    public class RopeTwistDemo : Demo
    {
        ...
        {
            renderer.TextBatcher.Write(text.Clear().Append("The ball is 10,000 times heavier than the rope bodies, and the ropes use no skip connections."), new Vector2(16, renderer.Height - 112), 16, Vector3.One, font);
            renderer.TextBatcher.Write(text.Clear().Append("This is intended as a worst case scenario simulation:"), new Vector2(16, renderer.Height - 96), 16, Vector3.One, font);
            renderer.TextBatcher.Write(text.Clear().Append("extremely high mass ratios, extremely high stiffness, extremely difficult to parallelize, no cheats."), new Vector2(16, renderer.Height - 80), 16, Vector3.One, font);
            renderer.TextBatcher.Write(text.Clear().Append("While this wouldn't be a very practical simulation for a game, it does work thanks to substepping!"), new Vector2(16, renderer.Height - 48), 16, Vector3.One, font);
            renderer.TextBatcher.Write(text.Clear().Append("This demo uses 60 substeps with 1 iteration each."), new Vector2(16, renderer.Height - 32), 16, Vector3.One, font);
            renderer.TextBatcher.Write(text.Clear().Append("Check the SubsteppingDemo, RopeStabilityDemo, and Substepping documentation for more information."), new Vector2(16, renderer.Height - 16), 16, Vector3.One, font);
