...
namespace Demos.Demos
{
    ...
    public class SolverContactEnumerationDemo : Demo
    {
        ...
        public override void Render(Renderer renderer, Camera camera, Input input, TextBuilder text, Font font)
        {
            ...
            renderer.TextBatcher.Write(text.Clear().Append("The solver stores data in an optimized array-of-structures-of-arrays format that makes it difficult to directly read."), new Vector2(16, renderer.Height - 96), 16, Vector3.One, font);
            renderer.TextBatcher.Write(text.Clear().Append("This demo implements an ISolverContactDataExtractor that pulls data out of the solver and puts it into a simpler format."), new Vector2(16, renderer.Height - 80), 16, Vector3.One, font);
            renderer.TextBatcher.Write(text.Clear().Append("Pulling data this way can sometimes be more convenient than tracking contacts from INarrowPhaseCallbacks."), new Vector2(16, renderer.Height - 64), 16, Vector3.One, font);
            renderer.TextBatcher.Write(text.Clear().Append("Contacts associated with the large box are visualized. The size of a contact corresponds to the contact's force."), new Vector2(16, renderer.Height - 48), 16, Vector3.One, font);
            renderer.TextBatcher.Write(text.Clear().Append("Sensor manifold constraint count: ").Append(extractor.Constraints.Count).Append(", contact count: ").Append(sensorContactCount), new Vector2(16, renderer.Height - 20), 20, Vector3.One, font);
