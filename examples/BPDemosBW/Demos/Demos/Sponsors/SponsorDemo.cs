...
namespace Demos.Demos.Sponsors
{
    ...
    public class SponsorDemo : Demo
    {
        ...
        async Task<RenderableImage> CreateRewardImageAsync(WebGLContext context, string rewardImagePath, ContentArchive content)
        {
            var textureContent = content.Load<Texture2DContent>(rewardImagePath);
            var image = new RenderableImage(context, textureContent);
            await image.InitializeAsync();
            return image;
        }
        async Task AddAsync(WebGLContext context, List<Sponsor> sponsors, string name, string rewardImagePath, ContentArchive content)
        {
            Sponsor sponsor;
            sponsor.RewardImage = await CreateRewardImageAsync(context, rewardImagePath, content);
            sponsor.Name = name;
            sponsors.Add(sponsor);
        }
        public override async Task LoadGraphicalContentAsync(WebGLContext context, ContentArchive content)
        {
            ...
            await AddAsync(context, sponsors1, "Y. V.", @"Content\Sponsors\smugturt.png", content);
            await AddAsync(context, sponsors1, "R. S. K.", @"Content\Sponsors\squidhanginout.png", content);
            await AddAsync(context, sponsors1, "Gavin-Williams", @"Content\Sponsors\angerybite.png", content);
            await AddAsync(context, sponsors1, "N.", @"Content\Sponsors\normalgiraffe.png", content);
            await AddAsync(context, sponsors1, "M. H.", @"Content\Sponsors\tarbeenus.png", content);
            await AddAsync(context, sponsors1, "K. C.", @"Content\Sponsors\physquirrel.png", content);
            await AddAsync(context, sponsors1, "P. S.", @"Content\Sponsors\borb.png", content);
            await AddAsync(context, sponsors1, "A different P. S.", @"Content\Sponsors\pleasedfrog.png", content);
            await AddAsync(context, sponsors1, "E. C.", @"Content\Sponsors\disturbingkoala.png", content);
            await AddAsync(context, sponsors1, "L. K. L.", @"Content\Sponsors\bork.png", content);
            await AddAsync(context, sponsors1, "A. L.", @"Content\Sponsors\waryoctopus.png", content);
            await AddAsync(context, sponsors1, "D. P.", @"Content\Sponsors\contentsluginthevoid.png", content);
            await AddAsync(context, sponsors1, "Creative-House.org", @"Content\Sponsors\handicat.png", content);
            await AddAsync(context, sponsors1, "vietnt", @"Content\Sponsors\raisondetre.png", content);
            await AddAsync(context, sponsors1, "demiurghg", @"Content\Sponsors\ifiwereatardigrade.png", content);
            ...
            await AddAsync(context, sponsors2, @"M. T.", @"Content\Sponsors\goose.png", content);
            await AddAsync(context, sponsors2, @"Yo Mamma wants you to go to bed", @"Content\Sponsors\spide.png", content);
            await AddAsync(context, sponsors2, @"A. R.", @"Content\Sponsors\scootybun.png", content);
            await AddAsync(context, sponsors2, @"Alex 'mcmonkey' Goodwin, @mcmonkey4eva on github and twitter", @"Content\Sponsors\mcmonkey.png", content);
            await AddAsync(context, sponsors2, @"A. K. D.", @"Content\Sponsors\behattedpenguin.png", content);
            await AddAsync(context, sponsors2, @"Cornmaiden xoxoxo", @"Content\Sponsors\healthyostrich.png", content);
            await AddAsync(context, sponsors2, @"F.", @"Content\Sponsors\tootbush.png", content);
            ...
            await AddAsync(context, sponsors3, @"Neos VR", @"Content\Sponsors\spooky.png", content);
        }

        Vector2 newtArenaMin, newtArenaMax;
        private readonly Random random = new(6);
        private readonly CharacterControllers characterControllers;
        QuickList<SponsorCharacterAI> characterAIs;

        public SponsorDemo() => characterControllers = new(BufferPool);
        ...
        public override void Render(Renderer renderer, Camera camera, Input input, TextBuilder text, Font font)
        {
            ...
            DrawSponsors("Sponsors", sponsors1, new Vector2(renderer.Width - 512, resolution.Y - 480), mousePosition, renderer, text, font, realTime, 4, 4, 0.25f, 24, 4, 36);
            DrawSponsors("Smaller sponsors who are still cool", sponsors0, new Vector2(renderer.Width - 512, resolution.Y - 288), renderer, text, font, realTime, 4, 4, 0.25f, 16, 24);
        ...
        protected override async ValueTask OnDisposeAsync()
        {
            ...
            {
                await sponsor.RewardImage.DisposeAsync();
            }
