            ...
            while (true)
            {
                ...
                if (font.Content.Characters.TryGetValue(character, out var characterData))
                {
                    ...
                    if (character != ' ')
                    {
                        ...
                        var id = font.GetSourceId(character);
                        glyph = new GlyphInstance(ref minimum, ref horizontalAxis, scale, id, ref color, ref screenToPackedScale, ref font.Sources[id]);
                    }
