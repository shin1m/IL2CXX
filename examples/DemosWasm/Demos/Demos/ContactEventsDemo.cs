...
namespace Demos.Demos
{
    ...
    public class ContactEvents : IDisposable
    {
        ...
        public void Dispose()
        {
            ...
            if (simulation != null) simulation.Timestepper.BeforeCollisionDetection -= SetFreshnessForCurrentActivityStatus;
            if (pendingWorkerAdds != null)
                for (int i = 0; i < pendingWorkerAdds.Length; ++i)
                {
    ...
    public class ContactEventsDemo : Demo
    {
        ...
        public override void Render(Renderer renderer, Camera camera, Input input, TextBuilder text, Font font)
        {
            ...
            renderer.TextBatcher.Write(text.Clear().Append("The library does not have a built-in concept of contact events like 'contact added' and 'contact removed'."), new Vector2(16, renderer.Height - 80), 16, Vector3.One, font);
            renderer.TextBatcher.Write(text.Clear().Append("The INarrowPhaseCallbacks interface exposes callbacks that can be used to create such events."), new Vector2(16, renderer.Height - 64), 16, Vector3.One, font);
            renderer.TextBatcher.Write(text.Clear().Append("This demo shows how events could be implemented. Green particles are spawned on contact add."), new Vector2(16, renderer.Height - 48), 16, Vector3.One, font);
            renderer.TextBatcher.Write(text.Clear().Append("The full list of events supported in the demo's source:"), new Vector2(16, renderer.Height - 32), 16, Vector3.One, font);
            renderer.TextBatcher.Write(text.Clear().Append("OnContactAdded, OnContactRemoved, OnStartedTouching, OnTouching, OnStoppedTouching, OnPairCreated, OnPairUpdated, and OnPairEnded."), new Vector2(16, renderer.Height - 16), 16, Vector3.One, font);
