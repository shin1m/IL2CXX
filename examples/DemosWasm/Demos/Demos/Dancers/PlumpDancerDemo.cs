...
namespace Demos.Demos.Dancers
{
    ...
    public class PlumpDancerDemo : Demo
    {
        ...
        static unsafe void CreateBodyGrid(DancerBodyHandles bodyHandles, Int3 axisSizeInBodies, Vector3 gridMinimum, Vector3 gridMaximum, float bodyRadius, float massPerBody,
        ...
        public override void Render(Renderer renderer, Camera camera, Input input, TextBuilder text, Font font)
        {
            ...
            renderer.TextBatcher.Write(text.Clear().Append("Cosmetic simulations, like character blubber, often don't need to be in a game's main simulation."), new Vector2(16, renderer.Height - 144), 16, Vector3.One, font);
            renderer.TextBatcher.Write(text.Clear().Append("Every background dancer in this demo has its own simulation. All dancers can be easily updated in parallel."), new Vector2(16, renderer.Height - 128), 16, Vector3.One, font);
            renderer.TextBatcher.Write(text.Clear().Append("Dancers further from the main dancer use sparser body grids and disable self collision for extra performance."), new Vector2(16, renderer.Height - 112), 16, Vector3.One, font);
            renderer.TextBatcher.Write(text.Clear().Append("Dancer count: ").Append(dancers!.Handles!.Length), new Vector2(16, renderer.Height - 80), 16, Vector3.One, font);
            renderer.TextBatcher.Write(text.Clear().Append("Total deformable body count: ").Append(dancers.BodyCount), new Vector2(16, renderer.Height - 64), 16, Vector3.One, font);
            renderer.TextBatcher.Write(text.Clear().Append("Total deformable constraint count: ").Append(dancers.ConstraintCount), new Vector2(16, renderer.Height - 48), 16, Vector3.One, font);
            renderer.TextBatcher.Write(text.Clear().Append("Total dancer execution time (ms): ").Append(dancers.ExecutionTime * 1000, 2), new Vector2(16, renderer.Height - 32), 16, Vector3.One, font);
            renderer.TextBatcher.Write(text.Clear().Append("Amortized execution time per dancer (us): ").Append(dancers.ExecutionTime * 1e6 / dancers.Handles.Length, 1), new Vector2(16, renderer.Height - 16), 16, Vector3.One, font);

            base.Render(renderer, camera, input, text, font);
        }
        protected override void OnDispose() => dancers!.Dispose(BufferPool);
    }
}
