...
namespace Demos.Demos
{
    ...
    public class RagdollTubeDemo : Demo
    {
        ...
        {
            renderer.TextBatcher.Write(text.Clear().Append("Ragdoll count:"), new Vector2(16, renderer.Height - 64), 16, Vector3.One, font);
            renderer.TextBatcher.Write(text.Clear().Append("Ragdoll body count:"), new Vector2(16, renderer.Height - 48), 16, Vector3.One, font);
            renderer.TextBatcher.Write(text.Clear().Append("Ragdoll constraint count:"), new Vector2(16, renderer.Height - 32), 16, Vector3.One, font);
            renderer.TextBatcher.Write(text.Clear().Append("Collision constraint count:"), new Vector2(16, renderer.Height - 16), 16, Vector3.One, font);
            const float xOffset = 192;
            renderer.TextBatcher.Write(text.Clear().Append(ragdollCount), new Vector2(xOffset, renderer.Height - 64), 16, Vector3.One, font);
            renderer.TextBatcher.Write(text.Clear().Append(ragdollBodyCount), new Vector2(xOffset, renderer.Height - 48), 16, Vector3.One, font);
            renderer.TextBatcher.Write(text.Clear().Append(ragdollConstraintCount), new Vector2(xOffset, renderer.Height - 32), 16, Vector3.One, font);
            var collisionConstraintCount = Simulation!.Solver.CountConstraints() - ragdollConstraintCount;
            renderer.TextBatcher.Write(text.Clear().Append(collisionConstraintCount), new Vector2(xOffset, renderer.Height - 16), 16, Vector3.One, font);
