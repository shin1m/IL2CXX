...
namespace Demos.Demos
{
    ...
    {
        private readonly RolloverInfo rolloverInfo = new();
        ...
        {
            renderer.TextBatcher.Write(text.Clear().Append("Substepping makes the solver run multiple mini timesteps for each call to Simulation.Timestep."), new Vector2(16, renderer.Height - 160), 16, Vector3.One, font);
            renderer.TextBatcher.Write(text.Clear().Append("Substeps can make extreme mass ratios and difficult constraint articulations stable at low costs."), new Vector2(16, renderer.Height - 144), 16, Vector3.One, font);
            renderer.TextBatcher.Write(text.Clear().Append("Simulations with substepping can use fewer velocity iterations per substep while remaining stable."), new Vector2(16, renderer.Height - 128), 16, Vector3.One, font);
            renderer.TextBatcher.Write(text.Clear().Append("Some difficult simulations will be vastly cheaper when using substepping than just increasing velocity iterations."), new Vector2(16, renderer.Height - 112), 16, Vector3.One, font);
            renderer.TextBatcher.Write(text.Clear().Append("Try modifying the substep/iteration counts to observe the effect on simulation stability."), new Vector2(16, renderer.Height - 96), 16, Vector3.One, font);
            renderer.TextBatcher.Write(text.Clear().Append("Check the Substepping documentation for more information."), new Vector2(16, renderer.Height - 80), 16, Vector3.One, font);
            renderer.TextBatcher.Write(text.Clear().Append("Substep count: ").Append(Simulation!.Solver.SubstepCount), new Vector2(16, renderer.Height - 48), 16, new Vector3(1), font);
            renderer.TextBatcher.Write(text.Clear().Append("Solver iteration count: ").Append(Simulation.Solver.VelocityIterationCount), new Vector2(16, renderer.Height - 32), 16, new Vector3(1), font);
            renderer.TextBatcher.Write(text.Clear().Append("Press Z/X to change substep count, C/V to change solver iteration count."), new Vector2(16, renderer.Height - 16), 16, new Vector3(1), font);
