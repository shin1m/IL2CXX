...
namespace System.Numerics.Tests
{
    public static class Util
    {
        ...
        public static T[] GenerateRandomValues<T>(int numValues, int min = 1, int max = 100) where T : INumber<T>
        {
        ...
        public static T GenerateSingleValue<T>(int min = 1, int max = 100) where T : INumber<T>
        {
            var randomRange = s_random.Next(min, max);
            //T value = unchecked((T)(dynamic)randomRange);
            T value = T.CreateTruncating(randomRange);
            return value;
        }
